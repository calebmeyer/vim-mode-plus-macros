...
export default {
  activate(_state) {
    ...
    const convertToCommand = (accumulator, character) => {
      ...
      accumulator[`vim-mode-plus-macros:record-insert-of-${character}`] = () => {
  ...
  startRecordingMacro() {
    let editorView = atom.views.getView(atom.workspace.getActiveTextEditor())
    editorView.classList.add('recording-macro')
  ...
  stopRecordingMacro() {
    let editorView = atom.views.getView(atom.workspace.getActiveTextEditor())
    editorView.classList.remove('recording-macro')
    recording = false
  ...
  applyMacro() {
    ...
    commandQueue.forEach(command => {
      console.log(command)
      let response = atom.commands.dispatch(atom.views.getView(this.activeEditor()), command)
  ...
  activeEditor() {
    ...
    if (editorView.classList.contains('search-pending')) {
      let allEditors = atom.textEditors.editorsWithMaintainedConfig
